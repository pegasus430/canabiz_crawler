class DispLeafly < ActiveJob::Base
	...
	def scrapeLeafly()
		...
		contents[@state_abbreviation].each do |returned_dispensary_source|
			...
			logger.info returned_dispensary_source["name"]
		...
		returned_json_menu.each do |returned_menu_section|
			...
			returned_menu_section['items'].each do |returned_dispensary_source_product|
				...
				if existing_dispensary_source_products.size > 0 #dispensary source has the product
					...
					#we have to check if the alternate names for the product matches
	...
	def createProductAndDispensarySourceProduct(product, dispensary_source_id, returned_dispensary_source_product, category_id)
		...
		if returned_dispensary_source_product['prices'] != nil
			DispensarySourceProduct.create(:product_id => product.id, 
				...
				:price => returned_dispensary_source_product['prices']['One'].to_s.tr('$', '').to_f, 
				:price_80mg => returned_dispensary_source_product['prices']['80mg'].to_s.tr('$', '').to_f,
				:price_160mg => returned_dispensary_source_product['prices']['160mg'].to_s.tr('$', '').to_f,
				:price_180mg => returned_dispensary_source_product['prices']['180mg'].to_s.tr('$', '').to_f,
				:price_100mg => returned_dispensary_source_product['prices']['100mg'].to_s.tr('$', '').to_f,
				:price_40mg => returned_dispensary_source_product['prices']['40mg'].to_s.tr('$', '').to_f,
				:price_25mg => returned_dispensary_source_product['prices']['25mg'].to_s.tr('$', '').to_f,
				:price_150mg => returned_dispensary_source_product['prices']['150mg'].to_s.tr('$', '').to_f,
				:price_10mg => returned_dispensary_source_product['prices']['10mg'].to_s.tr('$', '').to_f,
				:price_50mg => returned_dispensary_source_product['prices']['50mg'].to_s.tr('$', '').to_f,
				:price_240mg => returned_dispensary_source_product['prices']['240mg'].to_s.tr('$', '').to_f,
				:price_1mg => returned_dispensary_source_product['prices']['1mg'].to_s.tr('$', '').to_f,
				:price_2_5mg => returned_dispensary_source_product['prices']['2.5mg'].to_s.tr('$', '').to_f,
				:price_500mg => returned_dispensary_source_product['prices']['500mg'].to_s.tr('$', '').to_f,
				:price_1000mg => returned_dispensary_source_product['prices']['1000MG'].to_s.tr('$', '').to_f,

				:price_half_gram => returned_dispensary_source_product['prices']['HalfGram'].to_s.tr('$', '').to_f,
				:price_gram => returned_dispensary_source_product['prices']['Gram'].to_s.tr('$', '').to_f,
				:price_two_grams => returned_dispensary_source_product['prices']['TwoGrams'].to_s.tr('$', '').to_f,
				:price_eighth => returned_dispensary_source_product['prices']['Eighth'].to_s.tr('$', '').to_f,
				:price_quarter => returned_dispensary_source_product['prices']['Quarter'].to_s.tr('$', '').to_f,
				:price_half_ounce => returned_dispensary_source_product['prices']['Half'].to_s.tr('$', '').to_f,
				:price_ounce => returned_dispensary_source_product['prices']['Ounce'].to_s.tr('$', '').to_f
			)
	...
	def createDispensaryAndDispensarySourceAndProducts(dispensary_id, returned_dispensary_source)
		...
		location = returned_dispensary_source['address'].present? ? returned_dispensary_source['address'] : ''
		
		#location = location != nil ? location += ', ' + returned_dispensary_source['city'] : returned_dispensary_source['city']
		#location = location != nil ? location += ', ' + returned_dispensary_source['state'] : returned_dispensary_source['state']
		#location = location != nil ? location += ', ' + returned_dispensary_source['zip_code'] : returned_dispensary_source['zip_code']
		
		#+ ', ' + returned_dispensary_source['city'] + ', ' + 
	...
	def compareAndUpdateDispensarySourceValues(returned_dispensary_source, existing_dispensary_source)
		...
		if returned_dispensary_source['city'].present?
			location += returned_dispensary_source['city'] + ', '
		end
		if returned_dispensary_source['state'].present?
			location += returned_dispensary_source['state'] + ', '
		end
		if returned_dispensary_source['zip_code'].present?
			location += returned_dispensary_source['zip_code']
		end
